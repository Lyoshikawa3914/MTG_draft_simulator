...
def rename_table():
    c = db_common.cursor()
    c.execute("DELETE FROM WAR_COMMONS WHERE id=1")
    c.close()
#rename_table()
...
def random_selection(packs):
    ...
    for pack in packs:
        c.execute("Select Name FROM WAR_COMMONS ORDER BY RANDOM() LIMIT 10")
        row = c.fetchall()
...
def python_str_to_rating(names):
    ...
    c = db_common.cursor()
    for name in names:
        c.execute("Select RATING FROM WAR_COMMONS WHERE NAME = ?", (name,))
        row = c.fetchall()
        row = [i[0] for i in row]
        integer = row[0]
